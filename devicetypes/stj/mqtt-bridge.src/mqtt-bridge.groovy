...
def parse(String description) {
    setNetworkAddress()
    def hexPort = Integer.toHexString("$settings.port".toString().toInteger())
    hexPort = "0000".substring(hexPort.length()) + hexPort.toUpperCase()
    //log.debug "hexPort : '${hexPort}'"
    log.debug "Parsing '${description}'"
    def msg = parseLanMessage(description)
    //log.debug "Parse Lan Message: ('${msg.headers}') '${msg}'"
    def msgPort = msg.headers.port ?: hexPort
    log.debug("msg port: '${msgPort}' , hexPort : '${hexPort}'")
   if (msgPort == hexPort)  {
