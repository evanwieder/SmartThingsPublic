...
@Field CAPABILITY_MAP = [
    ...
    "button": [
        ...
        action: "actionClickDoubleClickHold"
    ],
...
def bridgeHandler(evt) {
    log.debug "Received device event from bridge (raw) ${evt.value}"
    if (evt.value != 'null'){
       ...
       /*
       CAPABILITY_MAP.findAll {capability -> capability.value.attributes.contains(json.type)}.find {
     		def dev = settings[capability.key]?.find{x -> x.value.displayName.contains (json.name)}
     		if (!dev)
             {   
                 return false
             }
             else 
             {
					if (json.command == false) {
                            if (dev.getSupportedCommands().any {it.name == "setStatus"}) {
                                log.debug "Setting state ${json.type} = ${json.value}"
                                dev.setStatus(json.type, json.value)
                                state.ignoreEvent = json;
                            }
                        }
                        else {
                            def cap = CAPABILITY_MAP[capability.key]
                            if (cap.containsKey("action")) {
                                def action = cap["action"]
                    ...
                    if (device.displayName == json.name) {
                        if (json.command == false) {
                            ...
                            if (capability.containsKey("action")) {
                                ...
                                // Yes, this is calling the method dynamically
                                log.debug "Calling action '{$action}' with '{$json.value}'"
                                "$action"(device, json.type, json.value)
                            }
                        }
                    }
...
def actionClickDoubleClickHold(device, attribute, value) {
	if (value == "click") {
    	device.click()
